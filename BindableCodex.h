...
namespace Bind
{
	...
	{
	public:
		static std::shared_ptr<Bindable> Resolve(const std::string& key) noexcept(!IS_DEBUG)
		{
	...
	private:
		std::shared_ptr<Bindable> Resolve_(const std::string& key) const noexcept(!IS_DEBUG)
		{
