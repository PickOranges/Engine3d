...
namespace Rgph
{
	...
	void BlurOutlineRenderGraph::RenderShadowWindow(Graphics& gfx)
	{
		if (ImGui::Begin("Shadows"))
		{
			...
			bool biasChange = ImGui::SliderFloat("Post Bias", &ctrl["depthBias"], 0.0f, 0.1f, "%.6f", 1.f);
...

			{
				...
				bool slopeChange = ImGui::SliderFloat("Slope Bias", &slope, 0.0f, 100.0f, "%.4f", 1.f);
				bool clampChange = ImGui::SliderFloat("Clamp", &clamp, 0.0001f, 0.5f, "%.4f", 1.f);
	...
	void BlurOutlineRenderGraph::RenderKernelWindow(Graphics& gfx)
	{
		if (ImGui::Begin("Kernel"))
		{
			...
			bool sigChange = ImGui::SliderFloat("Sigma", &sigma, 0.1f, 1.0f);
