...
modelPath(path.string())
{
	...
	// phong technique
	{
		...
		// common (pre)
		vtxLayout.Append(hw3d::VertexLayout::Position3D);
		vtxLayout.Append(hw3d::VertexLayout::Normal);
		...
		// diffuse
		{
			...
			if (material.GetTexture(aiTextureType_DIFFUSE, 0, &texFileName) == aiReturn_SUCCESS)
			{
				hasTexture = true;
				shaderCode += "Dif";
				vtxLayout.Append(hw3d::VertexLayout::Texture2D);
		...
		// normal
		{
			if (material.GetTexture(aiTextureType_NORMALS, 0, &texFileName) == aiReturn_SUCCESS)
			{
				...
				shaderCode += "Nrm";
				vtxLayout.Append(hw3d::VertexLayout::Texture2D);
				vtxLayout.Append(hw3d::VertexLayout::Tangent);
				vtxLayout.Append(hw3d::VertexLayout::Bitangent);
...
std::shared_ptr<Bind::VertexBuffer> Material::MakeVertexBindable(Graphics& gfx, const aiMesh& mesh, float scale) const noexcept(!IS_DEBUG)
{
	...
	if (scale != 1.0f)
	{
		...
		{
			DirectX::XMFLOAT3& pos = vtc[i].Attr<hw3d::VertexLayout::ElementType::Position3D>();
			pos.x *= scale;
			pos.y *= scale;
			pos.z *= scale;
		}
