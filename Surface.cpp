...
Surface::Surface(unsigned int width, unsigned int height)
{
	...
	if (FAILED(hr))
	{
		throw Surface::ExceptionSurface(__LINE__, __FILE__, "Failed to initialize ScratchImage", hr);
	}
}
...
Surface Surface::FromFile(const std::string& name)
{
	...
	if (FAILED(hr))
	{
		throw Surface::ExceptionSurface(__LINE__, __FILE__, name, "Failed to load image", hr);
	}
	...
	if (scratch.GetImage(0, 0, 0)->format != format)
	{
		...
		if (FAILED(hr))
		{
			throw Surface::ExceptionSurface(__LINE__, __FILE__, name, "Failed to convert image", hr);
		}
...
void Surface::Save(const std::string& filename) const
{
	const auto GetCodecID = [](const std::string& filename) {
		...
		throw ExceptionSurface(__LINE__, __FILE__, filename, "Image format not supported");
	};
	...
	if (FAILED(hr))
	{
		throw Surface::ExceptionSurface(__LINE__, __FILE__, filename, "Failed to save image", hr);
	}
}
...
Surface::ExceptionSurface::ExceptionSurface(int line, const char* file, std::string note, std::optional<HRESULT> hr) noexcept
	:
	Exception(line, file),
	note("[Note] " + note)
{
	if (hr)
	{
		note = "[Error String] " + Exception::TranslateErrorCode(*hr) + "\n" + note;
	}
}

Surface::ExceptionSurface::ExceptionSurface(int line, const char* file, std::string filename, std::string note_in, std::optional<HRESULT> hr) noexcept
	:
	Exception(line, file)
{
	...
	if (hr)
	{
		note = "[Error String] " + Exception::TranslateErrorCode(*hr) + note;
	}
}

const char* Surface::ExceptionSurface::what() const noexcept
{
	std::ostringstream oss;
	oss << ExceptionBase::what() << std::endl << GetNote();
	whatBuffer = oss.str();
	return whatBuffer.c_str();
}

const char* Surface::ExceptionSurface::GetType() const noexcept
{
	return "Chili Surface Exception";
}

const std::string& Surface::ExceptionSurface::GetNote() const noexcept
{
	return note;
}