...
namespace Rgph {
		...
		FullscreenPass(std::move(name), gfx)
	{
		...
		RegisterSink(DirectBindableSink<Bind::RenderTarget>::Make("renderTarget", renderTarget));
		RegisterSink(DirectBindableSink<Bind::DepthStencil>::Make("depthStencil", depthStencil));
